...
from vi.priorityqueue import displayDelegateSelector, viewBoneSelector, moduleHandlerSelector
		...
		for boneName, boneInfo in self.structure:
			if "params" in boneInfo.keys() and isinstance(boneInfo["params"], dict):
				...
				if "frontend_default_visible" in params and params["frontend_default_visible"]:
					...
					wdg = viewBoneSelector.select(self.module, boneName, structure)

					if wdg is not None:
						self.nodeHeadline.appendChild(wdg(self.module, boneName, structure, self.data))
						hasDescr = True
